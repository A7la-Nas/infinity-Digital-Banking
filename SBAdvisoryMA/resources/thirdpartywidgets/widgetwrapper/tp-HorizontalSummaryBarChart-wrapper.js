var HorizontalSummaryBarChart = {
        ...
        function drawVisualization() {
            ...
            var observer = new MutationObserver(function () {
                Array.prototype.forEach.call(container.getElementsByTagName('rect'), function (rect) {
                    ...
                    if (customData.chartProperties.id === 'HorizontalBarAveragePayment') {
                        if (rect.getAttribute('fill') === 'none' && rect.getAttribute('x') === '0' && rect.getAttribute('y') === '0' && rect.getAttribute('width') === customData.chartProperties.width) {
                            rect.setAttribute('width', (parseInt(rect.getAttribute('width')) - 70).toString());
                            //Update SVG
                            var svg = document.querySelector('#' + customData.chartProperties.id + ' svg');
                            svg.setAttribute('width', (parseInt(svg.getAttribute('width')) - 70).toString());
    ...
    handleTooltip: function (customData, isUserAction) {
        ...
        var barUI = document.querySelectorAll('#' + customData.chartProperties.id + ' rect[fill="' + customData.chartProperties.colors[0].toLowerCase() + '"]');
        Array.prototype.forEach.call(barUI, function (bar) {
            if (!(bar.hasOwnProperty("logicalname"))) {
                // Customize the tooltip's position
                var xaxisValue = parseInt(bar.getAttribute("x"));
                var yaxisValue = parseInt(bar.getAttribute("y"));
                var widthValue = parseInt(bar.getAttribute("width"));
                if (customData.chartProperties.id === 'HorizontalBarAveragePayment') {
                    tooltip.style.top = yaxisValue - 100 + "px";
                    tooltip.style.left = (Math.round((widthValue + xaxisValue) / 2) + (25 + (widthValue < 150 ? 15 : 0))) + "px";
                } else {
                    tooltip.style.top = bar.getAttribute("y") - 70 + "px";
                    tooltip.style.left = (Math.round(bar.getAttribute("width") / 2) + (widthValue < 150 ? 30 : parseInt(tooltip.style.width) >= 276 ? -10 : 20)) + "px";
                }
            }
        });
    }
