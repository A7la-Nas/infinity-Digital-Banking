    ...
    return {
        ...
        setFileDetails: function(fileData) {
            ...
            if (fileData.bulkWireName) {
                ...
                this.view.flxFileDetailsHeader.setVisibility(false);
            } else {
        ...
        setTransactionsData: function(transactions) {
            ...
            if (transactions.sortConfig.searchString == "") {
                this.counts.totalCount = transactionsData.totalCount;
                this.counts.failedCount = transactionsData.failedCount;
                this.counts.successCount = transactionsData.successCount;
                this.counts.pendingCount = transactionsData.pendingCount;
            }
            var currBreakpoint = kony.application.getCurrentBreakpoint();
            var accessibilityConfig = CommonUtilities.getaccessibilityConfig();
            ...
            if (!transactionsData || (transactionsData.Domestic.length === 0 && transactionsData.International.length === 0)) {
                ...
                for(var i=0;i<transactionsData.Domestic.length;i++){
                    if(transactionsData.Domestic[i].status && transactionsData.Domestic[i].status.toLowerCase()=="pending"){
                        transactionsData.Domestic[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Pending;
                    }   
                    else if(transactionsData.Domestic[i].status && transactionsData.Domestic[i].status.toLowerCase()=="failed"){
                        transactionsData.Domestic[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Failed;
                    }
                    else if(transactionsData.Domestic[i].status && transactionsData.Domestic[i].status.toLowerCase()=="denied"){
                        transactionsData.Domestic[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Denied;
                    }
                    else if(transactionsData.Domestic[i].status && transactionsData.Domestic[i].status.toLowerCase()=="executed"){
                        transactionsData.Domestic[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Executed;
                    }
                    else if(transactionsData.Domestic[i].status && transactionsData.Domestic[i].status.toLowerCase()=="rejected"){
                        transactionsData.Domestic[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Rejected;
                    }
                }
                for(var i=0;i<transactionsData.International.length;i++){
                    if(transactionsData.International[i].status && transactionsData.International[i].status.toLowerCase()=="pending"){
                        transactionsData.International[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Pending;
                    }   
                    else if(transactionsData.International[i].status && transactionsData.International[i].status.toLowerCase()=="failed"){
                        transactionsData.International[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Failed;
                    }
                    else if(transactionsData.International[i].status && transactionsData.International[i].status.toLowerCase()=="denied"){
                        transactionsData.International[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Denied;
                    }
                    else if(transactionsData.International[i].status && transactionsData.International[i].status.toLowerCase()=="executed"){
                        transactionsData.International[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Executed;
                    }
                    else if(transactionsData.International[i].status && transactionsData.International[i].status.toLowerCase()=="rejected"){
                        transactionsData.International[i].status = ViewConstants.BULKWIRE_STATUS_TYPES.Rejected;
                    }
                }
