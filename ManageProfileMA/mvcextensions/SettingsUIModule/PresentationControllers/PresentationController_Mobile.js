define(["CommonsMA/AsyncManager/BusinessControllers/BusinessController", "OLBConstants","CommonUtilities"], function(AsyncManager, OLBConstants,CommonUtilities) {
  function Settings_PresentationController() {
    ...
    this.PhoneTypes = {
      'Mobile': kony.i18n.getLocalizedString("kony.mb.ProfilePersonalDetails.Mobile"),
      'Work': kony.i18n.getLocalizedString("i18n.common.Work"),
      'Home': kony.i18n.getLocalizedString("kony.tab.common.home"),
      'Other': kony.i18n.getLocalizedString("i18n.cardsManagement.others")
    };
  ...
  Settings_PresentationController.prototype.getPasswordRulesAndPolicySuccessCallback = function(res){
    ...
    var data = navManager.getCustomInfo("frmProfileChangeandUpdatePasswordHID");
    } else if(CommonUtilities.getSCAType() == 2){
    var data = navManager.getCustomInfo("frmProfileChangeandUpdatePasswordUniken");
    } else {
    ...
    if(CommonUtilities.getSCAType() == 1){
    navManager.setCustomInfo("frmProfileChangeandUpdatePasswordHID",data);
    var validationUtility = applicationManager.getValidationUtilManager();
    validationUtility.createRegexForPasswordValidation(res.passwordrules);
    scope_SettingsPresenter.navigateToChangePassword();
    } else if(CommonUtilities.getSCAType() == 2){
    navManager.setCustomInfo("frmProfileChangeandUpdatePasswordUniken",data);
    var validationUtility = applicationManager.getValidationUtilManager();
    validationUtility.createRegexForPasswordValidation(res.passwordrules);
    scope_SettingsPresenter.navigateToChangePassword();
    } else{
  ...
  Settings_PresentationController.prototype.presentationAccountsSucc = function(res){
  ...
  Settings_PresentationController.prototype.getSavingsAndCheckingsAccounts = function (fromAccounts) {
  ...
  Settings_PresentationController.prototype.validatePassword = function(password){
    ...
    if(validationUtility.isPasswordValidForPolicy(password) === false){
     if(CommonUtilities.getSCAType() == 1){
      var formController = applicationManager.getPresentationUtility().getController('frmProfileChangeAndUpdatePasswordHID',true);
      formController.showFlxSecurityRequirements();
      } else if(CommonUtilities.getSCAType() == 2){
      var formController = applicationManager.getPresentationUtility().getController('frmProfileChangeAndUpdatePasswordUniken',true);
      formController.showFlxSecurityRequirements();
      } else {
  ...
  Settings_PresentationController.prototype.navigateToProfileEntity = function(){
    var navigationManager = applicationManager.getNavigationManager();
    navigationManager.navigateTo('SettingsUIModule/frmSettingsEntityPreferences');
  };
  ...
  Settings_PresentationController.prototype.getUserAllAddressesSuccess = function(data){
    ...
    for(var i =0;i<data.length;i++){
      ...
      if (data[i].isPrimary === "true") {
        ...
        type = type + "("+kony.i18n.getLocalizedString("kony.mb.common.MarkedCommunicating")+")";
      }
      var addr = "";
  ...
  Settings_PresentationController.prototype.navigateToAddOrEditEmail = function(flow){
    try{
      ...
      if(configManager.isRBUser === "true" || configManager. isSMEUser === "true"){
        ...
        applicationManager.getPresentationUtility().dismissLoadingScreen();
        if (flow == "updatesuccess" || flow == "deletesuccess" || flow == "addsuccess") {
           var self = this;
           var ntf = new kony.mvc.Navigation({
                "appName": "ManageProfileMA",
                "friendlyName": "frmProfileEditEmails"
              });
        }
        ...
        for (var key in orgAccounts) {
          ...
          for(var i=0;i<orgAccounts[key].length;i++){
            ...
            if (orgAccounts[key][i].isPrimary === "true") {
              type = type + "("+kony.i18n.getLocalizedString("kony.mb.common.MarkedPrimary")+")";
  ...
  Settings_PresentationController.prototype.navigateToEditPhoneNumber = function(info){
    ...
    if(configManager.isRBUser === "true" || configManager. isSMEUser === "true"){
      ...
      for(var i=0;i<phoneNumbers.length;i++){
        ...
        var type = phoneNumbers[i].Extension===undefined?kony.i18n.getLocalizedString("kony.mb.ProfilePersonalDetails.Mobile"):this.PhoneTypes[phoneNumbers[i].Extension];
        if(phoneNumbers[i].isPrimary === "true"){
          type = type + "("+kony.i18n.getLocalizedString("kony.mb.common.MarkedPrimary")+")";
        }else if(phoneNumbers[i].countryType && phoneNumbers[i].countryType.toLowerCase() === "international"){
          type = type + '('+kony.i18n.getLocalizedString("i18n.ProfileManagement.International") + ')';
        }
        temp.id = phoneNumbers[i].id;
        temp.lblDetail = type;
        //temp.lblDetailValue = phoneNumbers[i].phoneNumber;
        temp.lblDetailValue = (phoneNumbers[i].Value.includes("-")) ? phoneNumbers[i].Value : phoneNumbers[i].phoneCountryCode + "-" + phoneNumbers[i].Value;
  ...
  Settings_PresentationController.prototype.navigateToEditOrAddAddress = function(){
    ...
    for(var i =0;i<addressObj.length;i++){
      var type = (this.AddressTypes[addressObj[i].AddressType] !==null && this.AddressTypes[addressObj[i].AddressType] !==undefined) ? this.AddressTypes[addressObj[i].AddressType] : "";
      if (addressObj[i].isPrimary === "true") {
        type = type + "("+kony.i18n.getLocalizedString("kony.mb.common.MarkedCommunicating")+")";
      }
      var details = (addressObj[i].AddressLine1 !==null && addressObj[i].AddressLine1 !==undefined) ?addressObj[i].AddressLine1 :"";
  ...
  Settings_PresentationController.prototype.showSettings = function()
  {
    if (CommonUtilities.getSCAType() == 2) {
        scope_SettingsPresenter.getDeviceAuthDetails();
    }
    scope_SettingsPresenter.getDevDetails();
  ...
  Settings_PresentationController.prototype.getDeviceAuthDetails = function() {
        let response = RDNAAPI.getDeviceAuthenticationDetails();
           let userObj = applicationManager.getUserPreferencesManager();
           kony.print(JSON.stringify(response));
           if (response[0].longErrorCode === 0) {
               let authenticationType = JSON.parse(response[1]).authenticationCapabilities[0].authenticationType;
               let flag = JSON.parse(response[1]).authenticationCapabilities[0].isConfigured;
               let authMode = ((authenticationType === 9 || authenticationType === 4 || authenticationType ===1 ) && flag === 1) ? "touchid" : "password" ;
               userObj.setDefaultAuthMode(authMode);
           }
    };
  ...
  Settings_PresentationController.prototype.updatePinSuccess= function(success){
    ...
    var navManager = applicationManager.getNavigationManager();
    var pingChangeSuccess=kony.i18n.getLocalizedString("konymb.settings.pinchangesuccess");
    var msgData={};
    msgData['popUpMsg']=pingChangeSuccess;
    navManager.setCustomInfo("frmPreferencesPin",msgData) ;
   if (CommonUtilities.getSCAType() == 1) {
            navManager.navigateTo({"appName": "ManageProfileMA","friendlyName": "SettingsHIDUIModule/frmPreferencesPinHID"});}
        else{
  ...
  Settings_PresentationController.prototype.defaultAccounts = function(){
    ...
    acctId = userObj.getDefaultAccountforQRPayments();
    var defaultQRPaymentsAcc  = accountObj.getInternalAccountByID(acctId);
    var data=[
      {"lblTitle": kony.i18n.getLocalizedString("i18n.TransfersEur.Tabs.Transfers"),"lblValue":defaultTransferAcc.nickName,"imgArrow":"chevron.png","lblAccId":defaultTransferAcc.accountID},
      {"lblTitle": kony.i18n.getLocalizedString("kony.mb.BillPay.BillPay"),"lblValue":defaultBillPayAcc.nickName,"imgArrow":"chevron.png","lblAccId":defaultBillPayAcc.accountID},
      {"lblTitle": kony.i18n.getLocalizedString("i18n.P2P.PayPersonP2P"),"lblValue":defaultPaymentsAcc.nickName,"imgArrow":"chevron.png","lblAccId":defaultPaymentsAcc.accountID},
      {"lblTitle": kony.i18n.getLocalizedString("kony.mb.transaction.cashWithdrawal"),"lblValue":defaultCardlessAcc.nickName,"imgArrow":"chevron.png","lblAccId":defaultCardlessAcc.accountID},
      {"lblTitle": kony.i18n.getLocalizedString("i18n.accounts.deposits"),"lblValue":defaultDepositAcc.nickName,"imgArrow":"chevron.png","lblAccId":defaultDepositAcc.accountID},
      {"lblTitle": kony.i18n.getLocalizedString("i18n.qrpayments.QRPayments"),"lblValue":defaultQRPaymentsAcc.nickName,"imgArrow":"chevron.png","lblAccId":defaultQRPaymentsAcc.accountID}
    ];
    return(data);
  };
...
*/
  ...
  Settings_PresentationController.prototype.getAlertcategoriesList = function(){
    applicationManager.getPresentationUtility().showLoadingScreen();
    var alertsManager = applicationManager.getAlertsManager();
    alertsManager.fetchAlertsCategory(this.getAlertCategoriesListPresentationSuccessCallback.bind(),this.getAlertcategoriesListPresentationErrorCallback.bind());
    //scope_SettingsPresenter.getAlertChannelsData();
  };
  Settings_PresentationController.prototype.getAlertCategoriesListPresentationSuccessCallback = function(resSuccess){
    // var controller = applicationManager.getPresentationUtility().getController('frmSettings', true);
    var controller = _kony.mvc.GetController('SettingsUIModule/frmSettings', true,{"appName" : "ManageProfileMA"});
    var alertLevel = resSuccess.alertConfiguration ? resSuccess.alertConfiguration[0].alertPreferenceView : null;
    scope_SettingsPresenter.alertsManager.setAlertLevelConfiguration(alertLevel);
  ...
  Settings_PresentationController.prototype.getAlertcategoriesListPresentationErrorCallback = function(reserr){
    applicationManager.getPresentationUtility().dismissLoadingScreen();
    if (reserr["isServerUnreachable"]) {
      applicationManager.getPresentationInterruptHandler().showErrorMessage("postLogin", reserr);
    }else {
...
*/
 ...
 Settings_PresentationController.prototype.getTermsandConditionsSuccessCallBack = function(response){
  ...
  Settings_PresentationController.prototype.disableEBankingAccess = function(entities,selectedEntities){
    var userPrefManager = applicationManager.getUserPreferencesManager();
    var userName = userPrefManager.getUserObj().userName;
    var params = {
      "legalEntityList" : entities,
      "UserName": userName,
      "Status":"SUSPENDED"
    };
    userPrefManager.updateUserStatus(params,this.disableEBankingAccessSuccess.bind(this,selectedEntities),this.disableEBankingAccessError);
   };

   Settings_PresentationController.prototype.disableEBankingAccessSuccess = function(selectedEntities,response){
  ...
  Settings_PresentationController.prototype.mapAccountDetails = function(rowId, iban, rowIndexTppRes) {
   ...
   Settings_PresentationController.prototype.setSCAEnterPinFlowType= function(type){
    scope_SettingsPresenter.SCAEnterPinFlowType = type;
  };
   Settings_PresentationController.prototype.getSCAEnterPinFlowType= function(type){
    return scope_SettingsPresenter.SCAEnterPinFlowType;
   };
    Settings_PresentationController.prototype.fetchTermsAndConditions = function (request) {
      let scope = this;
      applicationManager.getPresentationUtility().showLoadingScreen();
      ...
      ], scope.onCompletionOfFetchTermsAndConditions.bind(this, request));
    };
  
    Settings_PresentationController.prototype.onCompletionOfFetchTermsAndConditions = function (request, syncResponseObject) {
      let scope = this;
      ...
      if (syncResponseObject.responses[0].isSuccess) {
        ...
        if (response.alreadySigned === "true") {
          scope.updateDefaultEntity(request);
        } else {
          response.id = request.id;
          navManager.setCustomInfo("frmSettingsEntityTermsAndConditions", response);
          scope.navigateToSettingsEntityTermsAndConditionsForm();
        }
      } else {
        applicationManager.getPresentationUtility().dismissLoadingScreen();
      }
    ...
    Settings_PresentationController.prototype.updateTermsAndConditions = function (request) {
      let scope = this;
      applicationManager.getPresentationUtility().showLoadingScreen();
      let asyncManager = applicationManager.getAsyncManager();
      asyncManager.callAsync([
      ...
      ], scope.onCompletionOfUpdateTermsAndConditions.bind(this, request));
    };
  
    Settings_PresentationController.prototype.onCompletionOfUpdateTermsAndConditions = function (request, syncResponseObject) {
      let scope = this;
      if (syncResponseObject.responses[0].isSuccess) {
        let response = syncResponseObject.responses[0].data;
        scope.updateDefaultEntity(request);
    ...
    Settings_PresentationController.prototype.updateDefaultEntity = function (request) {
      let scope = this;
      let userId = applicationManager.getUserPreferencesManager().getUserId();
      applicationManager.getPresentationUtility().showLoadingScreen();
      let asyncManager = applicationManager.getAsyncManager();
      asyncManager.callAsync([
        asyncManager.asyncItem(applicationManager.getMultiEntityManager(), 'updateDefaultEntity', [{
          "defaultLegalEntity": request.id,
          "userId": userId
        }])
      ], scope.onCompletionOfUpdateDefaultEntity.bind(this, request));
    };
  
    Settings_PresentationController.prototype.onCompletionOfUpdateDefaultEntity = function (request, syncResponseObject) {
      let scope = this;
      let navManager = applicationManager.getNavigationManager();
      let currForm = {"appName": "ManageProfileMA", "friendlyName": "SettingsUIModule/frmSettingsEntityTermsAndConditions"};
      if (navManager.stack && navManager.stack.length > 0 && JSON.stringify(navManager.stack[navManager.stack.length - 1]) === JSON.stringify(currForm)) {
        let friendlyName = (currForm && currForm.appName) ? ((currForm.friendlyName) ? currForm.appName + "/" + currForm.friendlyName : JSON.stringify(currForm)) : JSON.stringify(currForm);
        navManager.removeFormIndex(friendlyName);
        navManager.stack.pop();
      }
      if (syncResponseObject.responses[0].isSuccess) {
        let response = syncResponseObject.responses[0].data;
        let defaultLegalEntity = applicationManager.getUserPreferencesManager().getDefaultLegalEntity();
        let isDefaultEntityPreviouslyPresent = false;
        if (!kony.sdk.isNullOrUndefined(defaultLegalEntity) && defaultLegalEntity !== "") {
          isDefaultEntityPreviouslyPresent = true;
        }
        response.isDefaultEntityPreviouslyPresent = isDefaultEntityPreviouslyPresent;
        applicationManager.getUserPreferencesManager().saveDefaultLegalEntity(request.id);
        navManager.setCustomInfo("frmSettingsEntityAcknowledgement", response);
        scope.navigateToSettingsEntityAcknowledgementForm();
      } else {
        applicationManager.getPresentationUtility().dismissLoadingScreen();
      }
    };
    ...
    Settings_PresentationController.prototype.navigateToSettingsForm = function () {
      let navManager = applicationManager.getNavigationManager();
      navManager.navigateTo({ "appName": "ManageProfileMA", "friendlyName": "frmSettings" });
    };

    Settings_PresentationController.prototype.navigateToSettingsEntityTermsAndConditionsForm = function () {
      let navManager = applicationManager.getNavigationManager();
      navManager.navigateTo({ "appName": "ManageProfileMA", "friendlyName": "SettingsUIModule/frmSettingsEntityTermsAndConditions" });
    };

    Settings_PresentationController.prototype.navigateToSettingsEntityAcknowledgementForm = function () {
      let navManager = applicationManager.getNavigationManager();
      navManager.navigateTo({ "appName": "ManageProfileMA", "friendlyName": "SettingsUIModule/frmSettingsEntityAcknowledgement" });
    };

    Settings_PresentationController.prototype.getLegalEntities = function() {
        applicationManager.getSettingsManager().fetchLegalEntities(this.getLegalEntitiesSuccess.bind(this), this.getLegalEntitiesFailure.bind(this));
    };
 
    Settings_PresentationController.prototype.getLegalEntitiesSuccess = function(response) {
      var navManager = applicationManager.getNavigationManager();
      navManager.setCustomInfo("frmEBankingAccess", response );
      scope_SettingsPresenter.commonFunctionForNavigation("frmEBankingAccess");     
    };

    Settings_PresentationController.prototype.getLegalEntitiesFailure = function(response) {
      applicationManager.getPresentationUtility().dismissLoadingScreen();
    };
