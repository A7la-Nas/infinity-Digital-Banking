...
var objService = CIBAObjSrv.getDataModel("SCADevice", "SCAUniken");
  ...
  AuthManager.prototype.getServerTimezoneOffset = function (params, presentationSuccessCallback, presentationErrorCallback) {
      var requestObject = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Application");
      requestObject.customVerb('getServerTimeZoneOffset', {}, completionCallback);
      function completionCallback(status, data, error) {
        var serviceResponseHandler = applicationManager.getServiceResponseHandler();
        var responseObject = serviceResponseHandler.manageResponse(status, data, error, presentationSuccessCallback, presentationErrorCallback);
        var configManager = applicationManager.getConfigurationManager();
        if (responseObject["status"] === true) {
          configManager.constants.SERVER_TIMEZONE_DETAILS = responseObject["data"] || {};
        }
        else {
          configManager.constants.SERVER_TIMEZONE_DETAILS = {};
        }
      }
    };
