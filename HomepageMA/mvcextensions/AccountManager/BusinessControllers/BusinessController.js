 ...
 */
  ...
  /**
 * Fetches the Internal Customers using a service call.
 ...
 */
  AccountManager.prototype.fetchInternalCustomerAccounts = function(presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DigitalArrangements");
    accountsRepo.customVerb('getCoreCustomerIdsAndAccounts', {}, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
    ...
    /**
* Method to update Customers favourite status
* @param {JSON} params coreCustomer id for changing favourite status
* @param{function} presentationSuccessCallback - will be called when call is successfull.
* @param{function} presentationErrorCallback - will be called when call is not successfull.
*/
   AccountManager.prototype.updateCustomerFavouriteStatus = function(params,presentationSuccessCallback, presentationErrorCallback){
    ...
    var accountModel = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition("DigitalArrangements");
    accountModel.customVerb("updateCoreCustomerFavStatus",params, completionCallBack);
  };
  AccountManager.prototype.getInternalAccountsWithParams = function(params,presentationSuccessCallback, presentationErrorCallback){
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DigitalArrangements");
  ...
  function getLatestBalancesCompletionCallback(status, data, error) {
    ...
    var obj = srh.manageResponse(status, data, error);
    if (obj["status"] === true) {
      self.updateLatestBalances(obj["data"]);
...
//             configurationManager.setAccountPermissions(self.getAccountPermissionMap(obj["data"]["Accounts"]));
  ...
  AccountManager.prototype.updateLatestBalances = function(accounts) {
    var self = this;
    ...
    var configurationManager = applicationManager.getConfigurationManager();
    var fields = ['availableBalance','currentBalance','nextPaymentAmount','originalAmount','outstandingBalance','payoffAmount','principalBalance','principalValue'];
    internalAccounts = internalAccounts.map(internalAccount=>{
      var account = accountsMap[internalAccount['accountID']];
      for (key in fields){
        if (!kony.sdk.isNullOrUndefined(account) && account.hasOwnProperty(fields[key]) && internalAccount.hasOwnProperty(fields[key])){
          if(internalAccount[fields[key]]!==account[fields[key]]){
            internalAccount[fields[key]]=account[fields[key]];
          }
        }
      }
    ...
    self.clearInternalAccounts();
    self.splitInternalAccounts(internalAccounts);
    configurationManager.userAccounts = internalAccounts;
  };
 ...
 */
  ...
  AccountManager.prototype.retrieveAttachments = function(reqParams, presentationSuccessCallback, presentationErrorCallback) {
 ...
 * @param {Function} presentationErrorCallback - will be called when call is not successfull.
 */
  AccountManager.prototype.getCustomViews = function(params, presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("CustomView");
    accountsRepo.customVerb('getView', params, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if (obj["status"] === true) {
 ...
 */
 ...
 AccountManager.prototype.fetchAccountTransactionByType = function (inputParams, presentationSuccessCallback, presentationErrorCallback) {
   ...
   /**
 * Fetches the Internal Account details based on core customerId using a service call.
 * @param {object} params containing account Id.
 * @param {Function} presentationSuccessCallback - will be called when call is successful.
 * @param {Function} presentationErrorCallback - will be called when call is not successful.
 */
  AccountManager.prototype.fetchInternalAccountBycustID = function(params,presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DigitalArrangements");
    accountsRepo.customVerb('getList', params , getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
  ...
  /**
   * Method to fetch the Cash Flow Details for Smart Banking Advisory
   */
  AccountManager.prototype.fetchCashFlow = function (params, presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
  ...
  /**
   * Method to fetch the Health Score Details for Smart Banking Advisory
   */
  AccountManager.prototype.fetchHealthScore = function (params, presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var healthScoreRepo = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition("BusinessHealth");
    healthScoreRepo.customVerb('getScoreAndDrivers', params, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error, presentationSuccessCallback, presentationErrorCallback);
      if (obj["status"] === true) {
  ...
  /**
   * Method to update sba status based on user action, in case of SBA
   */
  AccountManager.prototype.updateSBAStatus = function (params, presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var healthScoreRepo = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition("SBAEnrollmentStatus");
    healthScoreRepo.customVerb('updateSBAStatus', params, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error, presentationSuccessCallback, presentationErrorCallback);
      if (obj["status"] === true) {
  ...
  AccountManager.prototype.saveActionsPermissionsInCache = function(params,presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DigitalArrangements");
    accountsRepo.customVerb('saveActionsPermissionsInCache', params , getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if (obj["data"]) {
  ...
  AccountManager.prototype.fetchInternalAccountsActions = function(params,presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DigitalArrangements");
    accountsRepo.customVerb('getAccountFeaturesAndPermissions', params , getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if (obj["data"]) {
  ...
  /**
    * Method to get enrolment status for SBA
    * @param {object} params - consist of payload to to get feature level permissions for SBA
    * @param {function} presentationSuccessCallback - invoke the call back with success response
    * @param {function} presentationErrorCallback - invoke the call back with error response
    */
  AccountManager.prototype.getEnrolmentStatus = function (params, presentationSuccessCallback, presentationErrorCallback) {
    var infinityUserModel = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition("Enrollment");
    infinityUserModel.customVerb("getStatus", params, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if (obj["status"] === true) {
  ...
  /**
  * Method to get SBA feature details
  * @param {object} params - consist of payload to to get feature details
  * @param {function} presentationSuccessCallback - invoke the call back with success response
  * @param {function} presentationErrorCallback - invoke the call back with error response
  */
  AccountManager.prototype.getSBAFeatureDetails = function (params, presentationSuccessCallback, presentationErrorCallback) {
    var infinityUserModel = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition("Enrollment");
    infinityUserModel.customVerb("getSBAFeaturesActions", params, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if (obj["status"] === true) {
  ...
  AccountManager.prototype.createEarlypayoffTermsAndConditions = function(presentationSuccessCallback, presentationErrorCallback) {
    var externalAccountRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Mortgage");
    externalAccountRepo.customVerb('createCustomerTNCForPayoffSimulation', {}, saveCompletionCallback);
    function saveCompletionCallback(status, data, error) {
        var srh = applicationManager.getServiceResponseHandler();
        var obj = srh.manageResponse(status, data, error);
        if (obj["status"] === true) {
  ...
  AccountManager.prototype.getAdhocStatements = function(criteria, presentationSuccessCallback, presentationErrorCallback) {
    var adhocStatements = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("AdhocStatements");
    adhocStatements.customVerb("getDetails", {}, getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
        var srh = applicationManager.getServiceResponseHandler();
        var obj = srh.manageResponse(status, data, error);
        if (obj["status"] === true) {
    ...
    /**
   * Method to fetch Unread Messages and Notifications and Outage messages.
   * @param {function} presentationSuccess -  will be called when call is successfull
   * @param {function} presentationError - will be called when call is not successfull
   */
    AccountManager.prototype.fetchMessagesNotifications = function(presentationSuccess,presentationError) {
      ...
      function completionCallback(status, data, error) {
        ...
        if (res['status']) {
          ...
          notificationManager.previousUnReadCount = notificationManager.unreadNotifications;
          notificationManager.unreadNotifications =  res.data['notifications'][0].UnreadNotificationCount;
          var messageManager = applicationManager.getMessagesManager();
          messageManager.unreadMessages = res.data.unreadMessageCount;
          messageManager.priorityMessageCount = res.data.priorityMessageCount;
          presentationSuccess(res['data']);
        } else {
          presentationError(res['errmsg']);
        }
      ...
      try {
        ...
        notificationsRepo.customVerb('GetMessagesNotifications', {}, completionCallback);
      } catch (error) {
        applicationManager
          .getLoggerManager()
          .log('Something went wrong in fetchOutageMessages');
